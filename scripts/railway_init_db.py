...
with app.app_context():
    ...
    for nome, unit, atual, minimo, custo, perda in insumos_base:
        if not Ingredient.query.filter_by(name=nome).first():
            db.session.add(Ingredient(
                ...
                cost_per_unit=custo,
                waste_percent=perda,
                supplier_id=fornecedor.id if fornecedor else None,
                active=True
            ))
    ...
    if Product.query.count() == 0:
        produtos = [
            Product(name="Smash Bairro", description="Pão brioche, burger smash, cheddar e molho da casa.", price=24.90, category_id=hamburguer.id, active=True, featured=True),
            Product(name="Duplo Chapa", description="Dois burgers smash, cheddar duplo, bacon crocante e barbecue.", price=34.90, category_id=hamburguer.id, active=True, featured=True),
            Product(name="Batata Cheddar Bacon", description="Batata crocante com cheddar cremoso e bacon.", price=19.90, category_id=acompanhamento.id, active=True, featured=True),
        ]
    ...
    db.session.commit()
    print("Banco preparado para produção.")
